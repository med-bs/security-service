...
public class AuthController {
    ...
    public AuthController(AccountService accountService, JwtDecoder jwtDecoder, AuthenticationManager authenticationManager) {
  ...
  */
    @PostMapping("/login")
    public ResponseEntity<Map<String, String>> requestForTokenLogIn(@RequestBody SignupDTO loginDTO) {
        Map<String, String> response;
        Authentication authentication = authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(
                        loginDTO.username(), loginDTO.password()
    ...
    public ResponseEntity<AppUser> findAppUser(@RequestHeader("Authorization") String bearerToken, @PathVariable String username) {
        String token;
    ...
    @PreAuthorize("hasAuthority('SCOPE_USER')")
    public ResponseEntity<AppUser> updateAppUser(@RequestHeader("Authorization") String bearerToken, @PathVariable String username, @RequestBody AppUser appUser) {
        String token;
        if (bearerToken.startsWith("Bearer ")) {
            token = bearerToken.substring(7);
            Jwt tokenDecoded = jwtDecoder.decode(token);
